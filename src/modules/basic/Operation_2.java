...
public class Operation_2{
    ...
    */ 
    ...
    /**
     * 用于对数计算的基准值，log10是要根据精度实时计算的，直接计算速度较慢，拆分成较小的数累加
     */
    private BigDecimal log10;
    /**
     * 用于对数计算的基准值，1.05/0.95的精确数值
     */
    private final BigDecimal _105_095;
    /**
     * 用于对数计算的基准值，ln(1.05/0.95)的精确数值，在后面简称ln(1.1)
     */
    ...
    **/
    ...
    /**
     * 快速计算，需要用到的函数，计算“当x∈[0.95,1.05]之间时”，lnx的值
    ...
    **/
    ...
    /**
     * 该函数调用log_095_105函数实现对(0,+无穷)范围的参数求ln(x)
     * 思路：先缩放到(0.5,5]，这里会用到ln10,然后再缩放到(0.95,1.05],这里会用到ln(1.05/0.95)
     * @param x
     * @return java.math.BigDecimal
     * @author 黄文杰
    ...
    **/
    ...
    public String getFac(String x)
    {
        ...
        {
            throw new ArithmeticException("小数无阶乘！");
        }
        return fac(X.intValue()).toString();
    }
    /**
     * 计算arctan(x)
     * 设置接口，接收string类型，返回string类型
     * @param x
